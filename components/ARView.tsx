...
const Heatmap3DView: React.FC<Heatmap3DViewProps> = ({ onClose, risks, seed: propSeed }) => {
  ...
  const fallbackRisks: RiskType[] = [
    { label: 'Confidentialité', score: 6, description: "L'accord permet la surveillance et le monitoring des communications numériques et de l'utilisation des appareils de l'employé." },
    { label: 'Options de Carrière', score: 8, description: "La clause de non-concurrence restreint la capacité de l'employé à travailler pour une entreprise concurrente." },
    { label: 'Sécurité d\'Emploi', score: 4, description: "La clause d'emploi à volonté permet à l'entreprise de mettre fin à l'emploi de l'employé à tout moment." }
  ];
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 10000, background: '#000' }}>
      ...
      <button onClick={onClose} style={{ position: 'absolute', top: 24, right: 24, zIndex: 10, background: '#222', color: '#fff', border: 'none', borderRadius: 8, padding: '12px 20px', fontSize: 18, cursor: 'pointer', opacity: 0.85, fontFamily: 'sans-serif' }}>Fermer la Visualisation</button>
    </div>
  );
};
