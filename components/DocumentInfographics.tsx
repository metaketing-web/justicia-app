...
const RISK_BUCKETS = [
  { label: 'Faible (0-3)', min: 0, max: 3 },
  { label: 'Moyen (4-7)', min: 4, max: 7 },
  { label: 'Élevé (8-10)', min: 8, max: 10 },
];

const SEVERITIES = ['Faible', 'Moyen', 'Élevé'];
const COLORS = ['#8B5CF6', '#facc15', '#ef4444'];
    ...
    <div className="flex flex-col items-center mt-4 mb-2 relative">
      <div className="relative" style={{ width, height }}>
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 text-white text-lg font-bold drop-shadow-lg tracking-wide" style={{ zIndex: 3, textShadow: '0 2px 8px #000a' }}>
          Carte des Points Chauds de Risque
        </div>
        {/* No glowing border */}
      </div>
...
const DocumentInfographics: React.FC<DocumentInfographicsProps> = ({ results }) => {
  ...
  return (
    <div className="flex flex-col gap-8 w-full max-w-full my-8 overflow-hidden" data-charts-container>
      {/* --- Density Heatmap FIRST --- */}
      <div className="bg-neutral-900 rounded-xl p-6 shadow-lg w-full max-w-full overflow-hidden" data-chart-type="density">
        <h4 className="text-lg font-bold text-white mb-4">Carte de Densité des Scores de Risque</h4>
        <DensityHeatmap risks={riskData} />
      </div>
      {/* --- Grid Heatmap --- */}
      <div className="bg-neutral-900 rounded-xl p-6 shadow-lg w-full max-w-full overflow-hidden" data-chart-type="grid">
        <h4 className="text-lg font-bold text-white mb-4">Carte de Chaleur des Scores de Risque (Grille)</h4>
        <HeatmapLegendBar />
      ...
      {/* --- Bar Chart --- */}
      <div className="bg-neutral-900 rounded-xl p-6 shadow-lg" data-chart-type="bar">
        <h4 className="text-lg font-bold text-white mb-4">Scores de Risque par Zone (Graphique en Barres)</h4>
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={riskData} margin={{ top: 16, right: 16, left: 0, bottom: 16 }}>
            <defs>
              <linearGradient id="justiciaBarGradient" x1="0" y1="0" x2="0" y2="1">
            ...
            <RechartsTooltip />
            <Bar dataKey="score" fill="url(#justiciaBarGradient)" radius={[8, 8, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
